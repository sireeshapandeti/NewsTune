...
### USA news table
def create_usanews_table():
    c.execute("CREATE TABLE IF NOT EXISTS usa_news(Title TEXT, PageLink TEXT, Article TEXT, News_Date INTEGER)")

# save articles data to politics table
def dynamic_news_entry():
    ...
    for i in range(len(Title)):
    	c.execute("INSERT INTO usa_news(Title, PageLink, Article, News_Date) VALUES (?, ?, ?, ?)", 
    		(str(Title[i]), str(PageLink[i]), str(Article[i]), str(News_Date[i])))    
    conn.commit()
'''
# Delete 1-day old data from usa_news table
def news_old_deletion():
	c.execute("DELETE FROM usa_news WHERE News_Date <= date('now', '-1 day')")
'''

create_usanews_table()
dynamic_news_entry()
#news_old_deletion()
