...
webpage = requests.get('https://www.cnn.com/entertainment') 
...
### Entertainment news table
def create_entertainment_table():
    c.execute("CREATE TABLE IF NOT EXISTS entertainment(Title TEXT, PageLink TEXT, Article TEXT, News_Date INTEGER)")

# save articles data to entertainment table
def dynamic_news_entry():
    ...
    for i in range(len(Title)):
    	c.execute("INSERT INTO entertainment(Title, PageLink, Article, News_Date) VALUES (?, ?, ?, ?)", 
    		(str(Title[i]), str(PageLink[i]), str(Article[i]), str(News_Date[i])))    
    conn.commit()
'''
# Delete 1-day old data from entertainment table
def news_old_deletion():
	c.execute("DELETE FROM entertainment WHERE News_Date <= date('now', '-1 day')")
'''
