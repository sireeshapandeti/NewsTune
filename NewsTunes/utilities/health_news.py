...
### CNN Business news
webpage = requests.get('https://www.cnn.com/health') 
...
def create_health_table():
    c.execute("CREATE TABLE IF NOT EXISTS health(Title TEXT, PageLink TEXT, Article TEXT, News_Date INTEGER)")

# save articles data to health table
def dynamic_news_entry():
    ...
    for i in range(len(Title)):
    	c.execute("INSERT INTO health(Title, PageLink, Article, News_Date) VALUES (?, ?, ?, ?)", 
    		(str(Title[i]), str(PageLink[i]), str(Article[i]), str(News_Date[i])))    
    conn.commit()

'''
# Delete 1-day old data from health table
def news_old_deletion():
	c.execute("DELETE FROM health WHERE News_Date <= date('now', '-1 day')")
'''
